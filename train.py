...
def construct_hyper_param(parser):
    ...
    parser.add_argument("--train_name", default='pro_esql_train.txt',
                        type=str, help="table_bert")
    parser.add_argument("--dev_name", default='pro_esql_dev.txt',
                        type=str, help="table_bert")
    parser.add_argument("--test_name", default='pro_esql_test.txt',
                        type=str, help="table_bert")
    parser.add_argument("--table_name", default='esql_table.json',
                        type=str, help="table_bert")
...
def train(train_loader, train_table, model, model_bert, opt, bert_config, tokenizer, epoch,
    ...
    if dset_name == 'train':
        ...
        for iB, t in enumerate(train_loader):
            ...
            if cnt < st_pos:
                continue
            # Get fields
            nlu, nlu_t, sql_i, sql_q, sql_t, tb, hs_t, hds, tid = get_fields(t, train_table, no_hs_t=True, no_sql_t=True)
            g_sc, g_sa, g_wn, g_wc, g_wo, g_wv, g_cond_conn_op, g_slen, idxs = get_g(sql_i)
            # g_wvi = get_g_wvi_corenlp(t, idxs)
            op_index = get_op_index(nlu,nlu_t)
            wemb_n, wemb_h, l_n, l_hpu, l_hs = get_wemb_bert(bert_config, model_bert, tokenizer, nlu_t,op_index, hds, max_seq_length,
            ...
            pr_sql_i = generate_sql_i(pr_sc, pr_scco, pr_sa, pr_wn, pr_wc, pr_wo, pr_wv_str, nlu, t, train_table, hds, tid)
        ...
        with torch.no_grad():
            ...
            for iB, t in enumerate(train_loader):
                ...
                #                     num_out_layers_n=num_target_layers, num_out_layers_h=num_target_layers)
                op_index = get_op_index(nlu,nlu_t)
                wemb_n, wemb_h, l_n, l_hpu, l_hs = get_wemb_bert(bert_config, model_bert, tokenizer, nlu_t, op_index, hds,
                                                                 max_seq_length,
                ...
                pr_wv_str, pr_wv_str_wp = convert_pr_wvi_to_string(pr_wvi, nlu_t, nlu)
                go_wv_str ,go_wv_str_wp = convert_pr_wvi_to_string(g_wvi, nlu_t, nlu)

                # print("agg:", pr_sa, ",cond_conn_op:", pr_scco, ",sel:", pr_sc, ",conds:" ,pr_wc, pr_wo, pr_wv_str)
                # print("@@@@@@@@@@@@@@")

                pr_sql_i = generate_sql_i(pr_sc, pr_scco, pr_sa, pr_wn, pr_wc, pr_wo, pr_wv_str, nlu, t, train_table, hds, tid)
                go_sql_i = generate_sql_i(g_sc, g_cond_conn_op, g_sa, g_wn, g_wc, g_wo, go_wv_str, nlu, t, train_table, hds, tid)
                # print(pr_sql_i)

                pre_sql_list.extend(pr_sql_i)
                gold_sql_list.extend(go_sql_i)
                ...
                # for i in range(len(cnt_wo1_list)):
                #     if cnt_lx1_list[i] == 0:
                #         print('question:', nlu[i])
                #         print('pre:', pr_sql_i[i])
                #         print('gold:', go_sql_i[i])
    ...
    return acc, aux_out, sql_acc, gold_sql_list, pre_sql_list
    ...
    for epoch in range(args.tepoch):
        ...
        if epoch>0:
            ...
            for p in opt_bert.param_groups:
                p['lr'] *= 0.9
        acc_train, aux_out_train, sql_acc_train, gold_sql_list, pre_sql_list = train(train_loader, tables, model, model_bert, opt, bert_config, tokenizer, epoch,
        ...
        acc_dev, aux_out_dev, sql_acc_dev, gold_sql_list, pre_sql_list = train(dev_loader, tables, model, model_bert, opt, bert_config, tokenizer, epoch,
                                      args.task, args.max_seq_length, args.num_target_layers, args.accumulate_gradients, start_time=None,
        ...
        if sql_acc_dev > sql_acc_best:
            ...
            acc_test, aux_out_test, sql_acc_test, gold_sql_list, pre_sql_list = train(test_loader, tables, model, model_bert, opt, bert_config, tokenizer, epoch,
                                      args.task, args.max_seq_length, args.num_target_layers, args.accumulate_gradients, start_time=None,
                                       heartbeat_hook=None, callconfig=None, opt_bert=opt_bert, st_pos=0, path_db=None, dset_name='dev')

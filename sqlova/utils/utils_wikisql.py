...
custom_labels = {
    ...
    '不叫': MY_POS_LABEL, 
    '以外': MY_POS_LABEL, 
    '除': MY_POS_LABEL, '等于': MY_POS_LABEL, '处于': MY_POS_LABEL, '位于': MY_POS_LABEL, '为': MY_POS_LABEL, '叫': MY_POS_LABEL, '属于': MY_POS_LABEL, '超过': MY_POS_LABEL, '高于': MY_POS_LABEL, '大于': MY_POS_LABEL, '高': MY_POS_LABEL, '低于': MY_POS_LABEL, '小于': MY_POS_LABEL, '低': MY_POS_LABEL
}
...
def get_fields(t1s, tables, no_hs_t=False, no_sql_t=False, testt=False):
    ...
    return nlu, nlu_t, sql_i, sql_q, sql_t, tb, hs_t, hs, tid

def get_op_index(nul, nlu_t):
    op_indexs = []
    for i, text in enumerate(nul):
        op_index = []
        unique_list = []
        doc = nlp(text)
        for token in doc:
            if token.text in custom_labels:
                token.tag_ = custom_labels[token.text]
        for token in doc:
            if token.tag_ == MY_POS_LABEL:
                start = nlu_t[i].index(token.text[0])
                end = start + len(token.text)
                op_index.append((start, end))
                unique_list = list(set(op_index))
        op_indexs.append(unique_list)
    return op_indexs
...
def get_bert_output(model_bert, tokenizer, nlu_t, hds, max_seq_length):
    ...
    for b, nlu_t1 in enumerate(nlu_t):
        ...
        if len(input_mask1) > max_seq_length:
...
def get_wemb_n(i_nlu, l_n, op_index, hS, num_hidden_layers, all_encoder_layer, num_out_layers_n):
    """
    ...
    for b in range(bS):
        ...
        for start, end in op_index[b]:
            wemb_n[b, start:end] += wemb_n[b, start:end]
    return wemb_n
    #
...
def get_wemb_bert(bert_config, model_bert, tokenizer, nlu_t, op_index, hds, max_seq_length, num_out_layers_n=1,
                  num_out_layers_h=1):
    # print('nlu_t:', nlu_t)
    # print('hds:', hds)
    ...
    wemb_n = get_wemb_n(i_nlu, l_n, op_index, bert_config.hidden_size, bert_config.num_hidden_layers, all_encoder_layer,
                        num_out_layers_n)
...
def generate_sql_i(pr_sc, pr_scco, pr_sa, pr_wn, pr_wc, pr_wo, pr_wv_str, nlu, t, table, hds, tid):
    # print("((((((")
    pr_sql_i = []
    for b, nlu1 in enumerate(nlu):
        ...
        if len(conds) == 1 and pr_wc[b][0] == len(tab['header'])-1:
            ...
            pr_scco[b] = 0
        pr_sql_i1 = {'agg': pr_sa[b], 'cond_conn_op': pr_scco[b], 'sel': pr_sc[b], 'conds': conds, 'header':hds[b], 'tid':tid[b]}
